...
const registerUser = asyncHandler( async (req,res)=>{
    ...
    if (
        // if anything is empty from this then return 
    ...
    const existedUser = await User.findOne({
        // this will return username or email
        // by this $or: we can write objects as many as we want in array by or operator
        $or: [{ username },{ email }]
    })
    if (existedUser){
    ...
    const user = await User.create({
        fullname, 
        avatar: avatar.url,
        // coverimage is not compulsary so we are checking
        coverImage: coverImage?.url || "",
        email,
        password,
        username: username.toLowerCase()
    })
    // if user is already exist then not select password and refreshToken by this syntax with space and -
